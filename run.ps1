    ...
    if ($isLogsCommand) {
        ...
        foreach ($lf in $logFiles) {
            ...
            foreach ($ev in $payload.events) {
                ...
                # Normalise sortable timestamp; fall back to file mtime if unparseable
        ...
        # Take the most recent N globally, then re-group by invokedFrom for display.
        ...
        foreach ($g in $groups) {
            # Show one version per group (most recent event's version wins; warn if the group spans versions)
